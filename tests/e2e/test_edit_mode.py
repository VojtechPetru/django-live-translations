...
class TestEditModeToggle:
    ...
    def test_hover_increases_outline_opacity(self, page_as_superuser: Page) -> None:
        activate_edit_mode(page_as_superuser)
        span = page_as_superuser.locator('.lt-translatable[data-lt-msgid="demo.title"]').first
        opacity_before = page_as_superuser.evaluate("el => getComputedStyle(el).outlineColor", span.element_handle())
        page_as_superuser.hover('.lt-translatable[data-lt-msgid="demo.title"]')
        opacity_after = page_as_superuser.evaluate("el => getComputedStyle(el).outlineColor", span.element_handle())
